...
export function normalizeOpenEmsPathname(pathname) {
  ...
  if (p === OPEN_EMS_ROUTES.admin) return OPEN_EMS_ROUTES.admin;
  if (p === OPEN_EMS_ROUTES.orderBess) return OPEN_EMS_ROUTES.orderBess;
  if (p === OPEN_EMS_ROUTES.buyHomeCharger)
    return OPEN_EMS_ROUTES.buyHomeCharger;
  if (p === OPEN_EMS_ROUTES.buyCommercialCharger)
    return OPEN_EMS_ROUTES.buyCommercialCharger;
  if (p === OPEN_EMS_ROUTES.rdnConsultation)
    return OPEN_EMS_ROUTES.rdnConsultation;
  if (
    p === OPEN_EMS_ROUTES.power ||
    p === LEGACY_POWER_PATH ||
    p === LEGACY_DAM_PATH
  ) {
...
export function resolveOpenEmsPage(pathname) {
  ...
  if (path === OPEN_EMS_ROUTES.admin) return "admin";
  if (path === OPEN_EMS_ROUTES.orderBess) return "orderBess";
  if (path === OPEN_EMS_ROUTES.buyHomeCharger) return "buyHomeCharger";
  if (path === OPEN_EMS_ROUTES.buyCommercialCharger)
    return "buyCommercialCharger";
