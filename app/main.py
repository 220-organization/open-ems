...
async def lifespan(app: FastAPI):
    ...
    stop_ev_port_power: Optional[asyncio.Event] = None
    ev_port_power_task: Optional[asyncio.Task[None]] = None
    if settings.EV_PORT_POWER_SNAPSHOT_ENABLED:
        stop_ev_port_power = asyncio.Event()
        ev_port_power_task = asyncio.create_task(ev_port_power_snapshot_loop(stop_ev_port_power))
        logger.info(
            "EV port power: snapshot to DB every %ss (EV_PORT_POWER_SNAPSHOT_*)",
            settings.EV_PORT_POWER_SNAPSHOT_INTERVAL_SEC,
        )
    ...
    stop_sc_auto_dam: Optional[asyncio.Event] = None
    sc_auto_dam_task: Optional[asyncio.Task[None]] = None
    if settings.DEYE_SELF_CONSUMPTION_AUTO_DAM_SCHEDULER_ENABLED:
        stop_sc_auto_dam = asyncio.Event()
        sc_auto_dam_task = asyncio.create_task(deye_self_consumption_auto_dam_loop(stop_sc_auto_dam))
        logger.info(
    ...
    if huawei_power_task is not None and stop_huawei_power is not None:
        ...
        try:
            ...
            pass
    if ev_port_power_task is not None and stop_ev_port_power is not None:
        stop_ev_port_power.set()
        ev_port_power_task.cancel()
        try:
            await ev_port_power_task
        except asyncio.CancelledError:
            pass
    ...
    if ev_port_task is not None and stop_ev_port is not None:
        ...
        try:
            ...
            pass
    if sc_auto_dam_task is not None and stop_sc_auto_dam is not None:
        stop_sc_auto_dam.set()
