...
async def get_soc_history_totals_route(
    deviceId: int = Query(..., ge=1, description="GridLab device id"),
    ...
    """Consumption / PV / grid-import totals for a Kyiv day/month/year (same shape as Deye)."""
    if not gridlab_configured():
        return JSONResponse(
            content={
                "ok": False,
    ...
    try:
        ...
        for day in period_days:
            _, hourly_grid_w, _, hourly_pv_kwh, hourly_load_kwh = await hourly_device_history_for_kyiv_day(
                db, int(deviceId), day
            )
            day_has_data = False
        ...
        logger.exception("GET /api/gridlab/soc-history-totals — failed: %s", exc)
        raise HTTPException(status_code=502, detail=str(exc)) from exc
