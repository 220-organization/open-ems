...
async def fetch_nbu_usd_row(date_compact: str) -> Optional[dict[str, Any]]:
    """Single-day NBU lookup for USD (YYYYMMDD)."""
    async with httpx.AsyncClient(timeout=20.0) as client:
        r = await client.get(
            NBU_EXCHANGE_URL,
            params={"valcode": "USD", "date": date_compact, "json": ""},
        )
        r.raise_for_status()
        data = r.json()
        if isinstance(data, list) and data and data[0].get("rate") is not None:
...
async def fetch_usd_uah_rate_for_date(preferred: date) -> Optional[float]:
    """
    Return UAH per 1 USD for preferred calendar day, or nearest earlier NBU publication (up to 10 days).
    """
    for i in range(10):
        d = preferred - timedelta(days=i)
        compact = d.strftime("%Y%m%d")
        try:
            row = await fetch_nbu_usd_row(compact)
        except httpx.HTTPError as e:
            logger.warning("NBU USD fetch failed for %s: %s", compact, e)
            return None
        if row is not None:
            rate = row.get("rate")
            if rate is not None and isinstance(rate, (int, float)):
